...
// ==========================================
// 14. MINECRAFT CRAFTING TABLE SYSTEM
// ==========================================

const craftGridState = Array(9).fill(null);
let selectedIngredient = null;

const CRAFTING_RECIPES = [
  {
    name: "HARV Autonomous Field Robot 🤖",
    reqs: ["Python", "ROS2"],
    icon: "🤖",
    desc: "1,000-lb GPS-RTK autonomous field robot developed at the USDA with 90% autonomy across 10+ acres!"
  },
  {
    name: "RegIntel Regulatory AI ⚡",
    reqs: ["FastAPI", "FAISS"],
    icon: "⚡",
    desc: "Domain RAG search engine over SEC 10-K filings & CFR Title 17 rules with hallucination guardrails!"
  },
  {
    name: "Brain MRI Tumor Segmenter 🩺",
    reqs: ["PyTorch", "U-Net"],
    icon: "🧠",
    desc: "Metadata-infused attention U-Net achieving 0.896 Dice score on LGG glioma brain MRI scans!"
  }
];
...
function pickIngredient(name, emoji) {
  if (name === "Clear") {
    craftGridState.fill(null);
    renderCraftGrid();
    return;
  }
  selectedIngredient = { name, emoji };
  play8BitSound("click");
}

function selectCraftSlot(idx) {
  if (selectedIngredient) {
    craftGridState[idx] = selectedIngredient;
    renderCraftGrid();
    play8BitSound("hotbar");
  }
}
...
function renderCraftGrid() {
  ...
  slots.forEach((slot, idx) => {
    const item = craftGridState[idx];
    slot.textContent = item ? item.emoji : "";
  });
}

function checkAndCraftRecipe() {
  const presentNames = craftGridState.filter(Boolean).map(i => i.name);
  const resultCard = document.getElementById("crafted-result-card");
  const outputIcon = document.getElementById("output-icon");

  let matched = null;
  for (const recipe of CRAFTING_RECIPES) {
    if (recipe.reqs.every(req => presentNames.includes(req))) {
      matched = recipe;
      break;
    }
  }
  ...
  if (matched) {
    play8BitSound("break");
    if (outputIcon) outputIcon.textContent = matched.icon;
    if (resultCard) {
      resultCard.style.display = "block";
      resultCard.innerHTML = `<strong>✨ CRAFTED: ${matched.name}</strong><br/>${matched.desc}`;
    }

    if (typeof unlockAdvancement === "function") {
      ADVANCEMENTS["master_craftsman"] = {
        icon: matched.icon,
        title: "Master Craftsman",
        type: "Secret Recipe Discovered!"
      };
      unlockAdvancement("master_craftsman");
    }
  } else {
    play8BitSound("mine");
    ...
    if (resultCard) {
      resultCard.style.display = "block";
      resultCard.innerHTML = `<strong>❌ No recipe match!</strong><br/>Try combining Python + ROS2, FastAPI + FAISS, or PyTorch + U-Net!`;
    }
  }
}
